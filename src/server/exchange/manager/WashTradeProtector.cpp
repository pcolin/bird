...
bool WashTradeProtector::TryAdd(const OrderPtr &order)
{
  ...
  if (order->IsBid())
  {
    auto it = std::find_if(asks.begin(), asks.end(), [&](const OrderPtr &ord)
        { return order->price >= ord->price; });
    if (it == asks.end())
    {
  ...
  {
    auto it = std::find_if(bids.begin(), bids.end(), [&](const OrderPtr &ord)
        { return order->price <= ord->price; });
    if (it == bids.end())
    {
