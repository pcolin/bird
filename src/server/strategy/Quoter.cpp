          ...
          if (itr->second->is_bid != sw->is_bid())
          {
            ...
            else if (itr->second->is_ask)
            {
              ...
              else if (side_quote_ && Check(inst, it->second->multiplier, itr->second))
              {
                ...
                else if(GetAskPrice(inst, theo, it->second->multiplier, itr->second->credit, price))
                {
                  bid.reset();
                  ask = NewOrder(inst, ask_, price);
                  api_->Submit(nullptr, ask);
...
void Quoter::Requote(const Instrument *inst, double spot, double multiplier,
...
bool Quoter::GetBidPrice(const Instrument *inst, double theo, double multiplier, double credit,
    double &price)
{
...
bool Quoter::GetAskPrice(const Instrument *inst, double theo, double multiplier, double credit,
    double &price)
{
...
void Quoter::SubmitOrders(const Instrument *inst, double theo, double multiplier,
