...
OrderDB::OrderDB(ConcurrentSqliteDB &db, InstrumentDB &instrument_db,
    ExchangeParameterDB &exchange_db)
  : DbBase(db, "Order" + exchange_db.TradingDay()), requests_(capacity_),
...
void OrderDB::RefreshCache()
{
  ...
  sprintf(sql, "CREATE TABLE '%s'(id varchar(24) PRIMARY KEY UNIQUE, instrument varchar(20), "
      ...
      "status integer, time varchar(30), latency integer)", table_name_.c_str());
  if (!ExecSql(sql))
  {
...
void OrderDB::Run()
{
  ...
  std::shared_ptr<Proto::OrderReq> requests[capacity_];
  while (true)
  {
