...
{
  auto *tmp = static_cast<std::tuple<QuoterMap*, QuoterRecordMap*, InstrumentDB*>*>(data);
  auto *quoters = std::get<0>(*tmp);
  auto *records = std::get<1>(*tmp);
  auto *instrument_db = std::get<2>(*tmp);
