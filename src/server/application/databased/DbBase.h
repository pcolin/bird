...
class ConcurrentSqliteDB {
 public:
  ...
  void BeginTransaction() { sqlite3_exec(db_, "begin;", 0, 0, 0); }
  void CommitTransaction() { sqlite3_exec(db_, "commit;", 0, 0, 0); }
...
class DbBase {
 ...
 protected:
  ...
  class TransactionGuard {
   public:
    TransactionGuard(DbBase *db) : db_(db) { db_->db_.BeginTransaction(); }
    ~TransactionGuard() { db_->db_.CommitTransaction(); }
