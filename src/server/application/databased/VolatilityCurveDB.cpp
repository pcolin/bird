...
VolatilityCurveDB::VolatilityCurveDB(ConcurrentSqliteDB &db,
...
base::ProtoMessagePtr VolatilityCurveDB::OnRequest(
  ...
  auto reply = std::make_shared<Proto::VolatilityCurveRep>();
  ...
  if (type == Proto::RequestType::Get) {
    ...
    for (auto &c : msg->curves()) {
      ...
      sprintf(sql, "INSERT OR REPLACE INTO %s VALUES('%s', '%s', %f, %f, %f, %f, %f, %f, %f, %f, %f,"
                   " %f, %f, %f, %f, %f)",
              table_name_.c_str(), c.underlying().c_str(), m.c_str(), c.spot(),
      ...
      auto it = vc.find(m);
      if (it == vc.end()) {
        vc.emplace(m, std::make_shared<Proto::VolatilityCurve>(c));
      } else {
...
int VolatilityCurveDB::Callback(void *data, int argc, char **argv, char **col_name) {
  ...
  if (inst) {
    auto vc = std::make_shared<Proto::VolatilityCurve>();
