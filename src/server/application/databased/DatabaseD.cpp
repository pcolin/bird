...
int main(int argc, char *argv[]) {
  ...
  QuoterDB quoter(config_db, "Quoter", "StrategyRecord", instrument_db);
  ok = ok && quoter.Initialize(dispatcher);

  HitterDB hitter(config_db, "Hitter", "StrategyRecord", instrument_db);
  ok = ok && hitter.Initialize(dispatcher);

  DimerDB dimer(config_db, "Dimer", "StrategyRecord", instrument_db);
  ok = ok && dimer.Initialize(dispatcher);

  HedgerDB hedger(config_db, "Hedger", instrument_db);
  ok = ok && hedger.Initialize(dispatcher);
